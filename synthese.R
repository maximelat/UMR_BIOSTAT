...
full<-function(Patients,Témoins,DATASET,chimio = FALSE , logi=false){
  ...
  if(!chimio){
    ...
    Student<-function(Patients,Témoins,Test.T,Test.pvalue, cutoff,logi){
      ...
      if(!logi){
        boxplot(Patients, Témoins,   main = paste("Boxplot Patient VS Témoins\n Moyenne et IC 95%"),names=c("Patients","Témoins"), xlab=paste("p-value de",Test.T,"=",format(as.numeric(Test.pvalue),scientific = T)),ylab = "intégrité")
      }
      else{
        boxplot(Patients, Témoins,   main = paste("Boxplot Patient VS Témoins\n Moyenne et IC 95%"),names=c("Patients","Témoins"), xlab=paste("p-value de",Test.T,"=",format(as.numeric(Test.pvalue),scientific=T)),ylab = "intégrité",log="y")
    ...
    boxplot(Patients, Témoins,   main = paste("Boxplot P1 VS P2 \n Moyenne et IC à 95%"),names=c("P1","P2"), xlab=paste("p-value de",Test.T,"=",format(as.numeric(Test.pvalue),scientific=T)),ylab = "intégrité",log="y")
